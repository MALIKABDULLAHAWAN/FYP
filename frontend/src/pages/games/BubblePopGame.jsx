...
export default function BubblePopGame() {
  ...
  useEffect(() => {
    ...
    spawnBubble();
    spawnRef.current = setInterval(spawnBubble, 850);
  ...
  const stars = score >= 30 ? 3 : score >= 15 ? 2 : 1;

  return (
    <div style={{ minHeight: "calc(100vh - 70px)", background: "linear-gradient(170deg,#87CEEB 0%,#98E4FF 45%,#B8F4FF 100%)", position: "relative", overflow: "hidden", fontFamily: "'Nunito','Inter',sans-serif" }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ position:"sticky", top:0, zIndex:200, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"11px 18px", background:"rgba(255,255,255,0.88)", backdropFilter:"blur(12px)", borderBottom:"2px solid rgba(77,150,255,0.18)" }}>
        <button onClick={() => navigate("/games")} style={{ background:"#FF6B6B", color:"white", border:"none", borderRadius:"50%", width:44, height:44, fontSize:22, cursor:"pointer", boxShadow:"0 4px 12px #FF6B6B44", lineHeight:1 }}>←</button>
        <div style={{ display:"flex", alignItems:"center", gap:18 }}>
          <div key={score} className="score-num" style={{ fontSize:26, fontWeight:900, color:"#4D96FF" }}>🫧 {score}</div>
          <div style={{ background:timeLeft < 10 ? "#FF6B6B" : "#6BCB77", color:"white", borderRadius:20, padding:"6px 16px", fontWeight:800, fontSize:20, minWidth:72, textAlign:"center", transition:"background 0.3s" }}>⏱ {timeLeft}</div>
        </div>
      </div>
      ...
      <div ref={areaRef} style={{ position:"relative", width:"100%", height:"calc(100vh - 135px)", overflow:"hidden" }}>
        ...
        {phase === "idle" && (
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:22, padding:24 }}>
            ...
            <div style={{ fontSize:18, color:"#555", textAlign:"center", maxWidth:280, lineHeight:1.5 }}>Tap the bubbles before they fly away!</div>
            <button onClick={startGame} style={{ background:"linear-gradient(135deg,#4D96FF,#6BCB77)", color:"white", border:"none", borderRadius:50, padding:"18px 56px", fontSize:28, fontWeight:900, cursor:"pointer", boxShadow:"0 8px 28px rgba(77,150,255,0.4)", marginTop:8 }}>
              ▶ Play!
            </button>
          </div>
        )}
        ...
        {/* Game over screen */}
        {phase === "over" && (
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:18, background:"rgba(255,255,255,0.96)", backdropFilter:"blur(10px)" }}>
            <div style={{ fontSize:80 }}>🎉</div>
            <div style={{ fontSize:32, fontWeight:900, color:"#4D96FF" }}>Great Job!</div>
            <div style={{ fontSize:52, letterSpacing:4 }}>{"⭐".repeat(stars)}</div>
            <div style={{ fontSize:22, fontWeight:700, color:"#555" }}>You popped <b style={{ color:"#FF6B6B" }}>{score}</b> bubbles!</div>
            <div style={{ display:"flex", gap:14, flexWrap:"wrap", justifyContent:"center", marginTop:8 }}>
              <button onClick={startGame} style={{ background:"linear-gradient(135deg,#FF6B6B,#FFD93D)", color:"white", border:"none", borderRadius:50, padding:"15px 38px", fontSize:22, fontWeight:900, cursor:"pointer", boxShadow:"0 6px 20px #FF6B6B44" }}>🔄 Again!</button>
              <button onClick={() => navigate("/games")} style={{ background:"linear-gradient(135deg,#4D96FF,#6BCB77)", color:"white", border:"none", borderRadius:50, padding:"15px 38px", fontSize:22, fontWeight:900, cursor:"pointer", boxShadow:"0 6px 20px #4D96FF44" }}>🏠 Games</button>
            </div>
          </div>
        )}
      </div>
