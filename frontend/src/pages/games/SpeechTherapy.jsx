...
function getMetadataService() {
  if (!metadataService) {
    metadataService = new GameMetadataService();
  }
...
export default function SpeechTherapy() {
  ...
  const [enrichedSession, setEnrichedSession] = useState(null);
  ...
  useEffect(() => {
    async function loadMetadata() {
      if (selectedActivity && currentActivity) {
        try {
          const metadata = await getMetadataService().getActivityMetadata(currentActivity.category);
          setActivityMetadata(prev => ({
            ...prev,
            [currentActivity.category]: metadata
          }));
        } catch (err) {
          console.warn('Failed to load activity metadata:', err);
        }
      }
    }
    loadMetadata();
  ...
  async function handleStartSession() {
    ...
    try {
      ...
      // Enrich session with metadata and initialize performance tracking
      if (childProfile && currentActivity) {
        try {
          const gameSession = {
            gameId: currentActivity.category,
          ...
          const enriched = await getMetadataService().enrichGameWithMetadata(gameSession, childProfile);
          setEnrichedSession(enriched);
          
          // Record session start for analytics
          await getMetadataService().recordSession({
            ...gameSession,
            startedAt: new Date().toISOString(),
            status: 'active'
          });
        } catch (metaErr) {
          console.warn('Failed to enrich session with metadata:', metaErr);
        }
      }
  ...
  async function handleScore(score) {
    ...
    try {
      ...
      // Record trial performance data
      if (enrichedSession) {
        try {
          await getMetadataService().recordSession({
            sessionId: session.session_id,
            gameId: enrichedSession.gameId,
            childId: enrichedSession.childId,
            trialIndex: currentTrialIndex,
            score,
            accuracy: score === 'success' ? 1 : score === 'partial' ? 0.5 : 0,
            responseTime: recordingTime,
            timestamp: new Date().toISOString()
          });
        } catch (perfErr) {
          console.warn('Failed to record performance data:', perfErr);
        }
      }
        ...
        if (enrichedSession) {
          try {
            await getMetadataService().recordSession({
              ...
              totalCompleted: sum.total_completed,
              correct: sum.correct,
            ...
            console.warn('Failed to record final session data:', perfErr);
          }
        }
        ...
        <div className="panel" style={{ maxWidth: 680 }}>
          ...
          {!selectedChild && (
            <div style={{
              ...
              <UiIcon name="toast-warning" size={22} title="" />
              <span>Please select a child from the <strong>Games</strong> page first.</span>
            </div>
          )}
          ...
          <div style={{
            ...
            {activities.map((a) => {
              ...
              const metadata = activityMetadata[a.category];
              return (
                <div
                  key={a.id}
                ...
                >
                  ...
                  {metadata && (
                    <div style={{ fontSize: 9, opacity: 0.6, marginTop: 4, color: '#818cf8' }}>
                      {metadata.therapeuticGoals?.length || 0} goals
                    </div>
                  )}
                </div>
              );
          ...
          {currentActivity && (
            <div style={{
              ...
              {activityMetadata[currentActivity.category] && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 4, color: '#a78bfa' }}>Therapeutic Goals:</div>
                  <div style={{ fontSize: 11, opacity: 0.8, lineHeight: 1.4 }}>
                    {activityMetadata[currentActivity.category].therapeuticGoals?.slice(0, 3).join(', ')}
                  </div>
                  {activityMetadata[currentActivity.category].evidenceBase?.length > 0 && (
                    <div style={{ fontSize: 10, opacity: 0.6, marginTop: 4 }}>
                      Evidence-based: {activityMetadata[currentActivity.category].evidenceBase.length} research studies
                    </div>
                  )}
                </div>
              )}
